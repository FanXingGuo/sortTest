...
func merge_pass(arr []int, len int, k int) {
	i := 0
	for i < len-2*k+1 {
...
func partion(arr []int, start int, end int) int {
	i, j := start-1, start
	for ; j < end; j++ {
		if arr[j] < arr[end] {
			i++
...
func adjuest(arr []int, length int, index int) {
	left := 2*index + 1
	right := 2*index + 2
	maxIdx := index
...
func BucketSort(arr []int, bktNum int) {
	length := len(arr)
...
func base_sort(arr []int, exp int) {
	length := len(arr)
	output := make([]int, length)
	buckets := make([]int, 10)
	for i := 0; i < length; i++ {
...
func workerMerge(id int, jobs <-chan [3]int, arr []int,wg *sync.WaitGroup) {
	defer wg.Done()
	indexs := <-jobs
...
func workerMerge2(indexs [3]int, arr []int,wg *sync.WaitGroup) {
	defer wg.Done()
	if !(indexs[0]==indexs[2]+1){
		Merge(arr, indexs[0], indexs[1], indexs[2])
	}
